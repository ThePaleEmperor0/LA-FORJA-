...
def cargar_accesorios(v):
    ...
    p = tk.Frame(v, bg=bg, bd=0, relief="flat", width=v.winfo_screenwidth(), height=v.winfo_screenheight())
    p.pack(fill="both", expand=True)

    pi = tk.Frame(p, bg=bg, bd=1, relief="flat", width=v.winfo_screenwidth()//2, highlightbackground=brd, highlightthickness=1)
    ...
    def act():
        try:
            base = conectar("SELECT producto FROM accesorios")
    ...
    pd = tk.Frame(p, bg=bg, bd=1, relief="flat", highlightbackground=brd, highlightthickness=1)
    ...
    cl = tk.Frame(pd, bg=brd, bd=1, relief="solid")
